...
class Runner(object):
    ...
    def play(self, player1, player2):
        ...
        while True:
            try:
                hits = player1.get_damage() - player2.get_armor()
                player2.take_hits(hits)

            except ExcpetionLost:
                print("player 2 lost")
                return player1

            try:
                hits = player2.get_damage() - player1.get_armor()
                player1.take_hits(hits)
            ...
            for armor in ARMOR:
                ...
                for ring1 in RINGS:
                    ...
                    for ring2 in RINGS:
                        ...
                        if winner == player1:
                            if total_cost < self._min_cost:
                                self._min_cost = total_cost
                        else:
                            if total_cost > self._max_cost:
                                self._max_cost = total_cost

        print("lowest winning cost", self._min_cost)
        print("highest losing cost", self._max_cost)
